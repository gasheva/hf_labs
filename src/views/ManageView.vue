<script setup lang="ts">
    ...
    const {isValid: isRecordValid, isTemperaturesValid, isIdExisted} = useRecord();
    ...
    const modal = ref<{isOpen: boolean, data: {text: string, callback: Function}}>({
        isOpen: false,
        data: {
            text: '',
            callback: ()=>{},
    ...
    if (id) {
        ...
        if(!searchingRecord) {
            ...
            temperatures.value = record.value.temperatures.map((t:number, idx:number) => ({
                id: idx,
                oldValue: t,
                value: t,
                isEdit: false
    ...
    const onCancel = () => {
        ...
        modal.value.data.text= "Вы уверены, что хотите отменить изменения и вернуться к списку?";
    ...
    const onSave = () => {
        ...
        if(mode.value===Modes.CREATE && isIdExisted(record.value.id, records.value)){
            errorSaving.value = "Id должен быть уникальным"
            return;
        }

        if(mode.value===Modes.EDIT && isIdExisted(record.value.id, records.value) && id!==record.value.id){
            errorSaving.value = "Id должен быть уникальным"
            return;
        }
        ...
        if(!validation.status){
            errorSaving.value = validation.message || "Проверьте правильность заполнения данных"
            return;
        }
...
<template>
    <div>
        <div v-if="!error" class="manage-view">
            <div class="record">
                <base-input v-if="record" :value="record.id" @update:value="onIdUpdate" label="Id:"/>
                <div class="temperatures">
                    <div v-for="t in temperatures" :key="t.id" class="temperatures__item">
                        <number-input
                                :value="t.value"
                                label="Температура:"
                                @update:value="(val)=>onTemperatureUpdate(t.id, val)"/>
                        <icon-button color="gray" @on-click="()=>onDelete(t.id)">
                            <template #default>
                                <font-awesome-icon icon="fa-regular fa-trash-can" />
                            </template>
                        </icon-button>
                        <icon-button v-if="t.isEdit && mode===Modes.EDIT" label="о" color="gray"
                    ...
                    <base-button class="button-add" color="gray" label="Добавить" @on-click="addTemperature"/>
                </div>
            </div>

            <div class="footer">
          <div class="footer__buttons">
                 <base-button color="gray" label="Отменить" @on-click="onCancel"/>
              <base-button :disabled="!wasChanged" color="blue" label="Сохранить" @on-click="onSave"/>
          </div>
                <span class="text text__error" v-show="errorSaving">{{ errorSaving }}</span>
            </div>
        </div>

        <div v-else class="error text text__error">{{ error }}</div>
        <base-modal v-model:is-open="modal.isOpen"
