...
const modal = ref<{isOpen: boolean, data: {id: string}}>({
    isOpen: false,
    data: {
        id: '',
    }
...
<template>
  <main class="home">
      <div class="buttons">
          <span>
              <BaseButton color="blue" label="Создать новую запись" type="info" @on-click="onAdd"/>
          </span>
      </div>
      <div class="records">
          <transition-group name="fade">
            <RecordItem v-for="record in records"
                      :key="record.id"
                      :id="record.id"
